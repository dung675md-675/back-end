...
class AuthIntegrationTests {
    ...
    @Test
    void staticPagesShouldBeAccessibleWithoutAuthentication() throws Exception {
        mockMvc.perform(get("/"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("/user/index.html")));
    ...
    @Test
    void customerShouldAccessOnlyOwnCustomerProfile() throws Exception {
        User owner = createCustomerUser("daisy", "Password123", "daisy@example.com");
        User other = createCustomerUser("eric", "Password123", "eric@example.com");

        String ownerToken = loginAndGetToken(owner.getUsername(), "Password123");

        mockMvc.perform(get("/api/customers/user/{userId}", owner.getId())
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.userId").value(owner.getId()));

        mockMvc.perform(get("/api/customers/user/{userId}", other.getId())
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isForbidden());
    }

    @Test
    void customerShouldAccessCurrentCustomerProfileViaMeEndpoint() throws Exception {
        User owner = createCustomerUser("frank", "Password123", "frank@example.com");
        String ownerToken = loginAndGetToken(owner.getUsername(), "Password123");
    ...
    private String loginAndGetToken(String username, String password) throws Exception {
        AuthDTO.LoginRequest request = new AuthDTO.LoginRequest(username, password);
