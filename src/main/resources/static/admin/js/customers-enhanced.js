...
let customersData = [];
let adminCoupons = [];
let selectedCustomerIdForCoupon = null;
                ...
                <div class="checkbox-grid">
                    ${CUSTOMER_LEVELS.map(level => `
                        <label class="level-option">
                            <input type="checkbox" name="targetLevels" value="${level}">
                            <span>
...
function injectAssignmentModal() {
    if (document.getElementById('customerCouponAssignModal')) return;

    const modal = document.createElement('div');
    modal.id = 'customerCouponAssignModal';
    modal.className = 'modal';
    modal.innerHTML = `
        <div class="assign-modal-box">
            <h3>Gắn voucher cho khách hàng</h3>
            <p id="assignCustomerSummary">Chọn một hoặc nhiều voucher để gắn cho khách hàng này.</p>
            <div id="customerCouponChecklist" class="checkbox-grid"></div>
            <div class="assign-modal-actions">
                <button class="btn btn-secondary" onclick="closeCustomerCouponAssignModal()">Đóng</button>
                <button class="btn btn-primary" onclick="submitCustomerCouponAssignment()">Gắn các voucher đã chọn</button>
            </div>
        </div>
    `;
    ...
    modal.addEventListener('click', event => {
        if (event.target === modal) {
            closeCustomerCouponAssignModal();
        }
    });
}
...
async function loadCouponsForAssignment() {
    try {
        adminCoupons = await fetchGet(`${API_BASE_URL}/coupons`);
    } catch (error) {
        adminCoupons = [];
    }
}
...
async function createAdminCustomerCoupon() {
    ...
    const targetLevels = Array.from(document.querySelectorAll('input[name="targetLevels"]:checked'))
    ...
    try {
        ...
        await Promise.all([loadCouponsForAssignment(), loadCustomers()]);
...
function formatCouponDescription(coupon) {
    const discount = coupon.discountLabel || (coupon.discountType === 'FIXED_AMOUNT'
        ? `Giảm ${formatCurrency(coupon.fixedDiscountAmount || 0)}`
        : `Giảm ${coupon.discountPercent || 0}%`);
    return `${coupon.name || coupon.code} (${coupon.code}) - ${discount}`;
}

function openCustomerCouponAssignModal(customerId) {
    selectedCustomerIdForCoupon = customerId;
    const customer = customersData.find(item => item.id === customerId);
    const summary = document.getElementById('assignCustomerSummary');
    const checklist = document.getElementById('customerCouponChecklist');

    if (!customer || !checklist) return;

    if (summary) {
        summary.textContent = `Chọn một hoặc nhiều voucher để gắn cho ${customer.fullName} (${CUSTOMER_LEVEL_META[customer.rank]?.label || customer.rank}).`;
    }

    const assignedCodes = new Set(customer.assignedCoupons || []);
    checklist.innerHTML = adminCoupons.length === 0
        ? '<div class="empty-badge">Chưa có voucher nào trong hệ thống.</div>'
        : adminCoupons.map(coupon => `
            <label class="coupon-option">
                <input type="checkbox" name="selectedCustomerCoupons" value="${coupon.id}" ${assignedCodes.has(coupon.code) ? 'checked' : ''}>
                <span>
                    <strong>${coupon.name || coupon.code}</strong>
                    <small>${coupon.code} - ${coupon.discountLabel || formatCouponDescription(coupon)}</small>
                </span>
            </label>
        `).join('');

    document.getElementById('customerCouponAssignModal')?.classList.add('show');
}

function closeCustomerCouponAssignModal() {
    selectedCustomerIdForCoupon = null;
    document.getElementById('customerCouponAssignModal')?.classList.remove('show');
}

async function submitCustomerCouponAssignment() {
    const couponIds = Array.from(document.querySelectorAll('input[name="selectedCustomerCoupons"]:checked'))
        .map(input => Number(input.value));

    if (!selectedCustomerIdForCoupon) {
        showNotification('Chưa chọn khách hàng để gắn voucher.', 'warning');
        return;
    }
    if (couponIds.length === 0) {
        showNotification('Vui lòng chọn ít nhất một voucher.', 'warning');
        return;
    }

    try {
        await fetchPost(`${API_BASE_URL}/coupons/assign/customer/bulk`, {
            customerId: selectedCustomerIdForCoupon,
            couponIds
        });
        closeCustomerCouponAssignModal();
        await loadCustomers();
        showNotification('Đã gắn các voucher đã chọn cho khách hàng.', 'success');
    } catch (error) {
        showNotification(error.message || 'Không thể gắn voucher cho khách hàng.', 'error');
    }
}
    ...
    tbody.innerHTML = customers.map(customer => `
        <tr>
            ...
            <td class="table-actions">
                ...
                <button class="btn btn-sm assign-btn" onclick="openCustomerCouponAssignModal(${customer.id})">Gắn voucher</button>
            </td>
        </tr>
    `).join('');
}
...
window.onclick = function(event) {
    ...
    const assignModal = document.getElementById('customerCouponAssignModal');
